...
def test_export_creates_complete_yolo_import_folder(tmp_path: Path) -> None:
    ...
    frame_metadata_path.write_text("\n".join(json.dumps(record) for record in [
        {"file_name": "one.jpg", "source_mcap_path": "/one.mcap", "source_mcap_stem": "one", "source_video": "one.mp4", "timestamp_seconds": 1.0},
        {"file_name": "two.jpg", "source_mcap_path": "/two.mcap", "source_mcap_stem": "two", "source_video": "two.mp4", "timestamp_seconds": 2.0},
    ]) + "\n")
    prediction_path.write_text("\n".join(json.dumps(record) for record in [
        {"file_name": "one.jpg", "width": 100, "height": 50, "detections": [{"category": "right_hand", "bbox_xyxy": [25, 10, 75, 40]}]},
        {"file_name": "two.jpg", "width": 100, "height": 50, "detections": []},
    ]) + "\n")
    output_dir = tmp_path / "roboflow"

    result = export_roboflow_yolo(frames_dir, frame_metadata_path, prediction_path, output_dir)

    assert (result.image_count, result.detection_count, result.empty_label_count) == (2, 1, 1)
    assert (output_dir / "images/train/one.jpg").read_bytes() == b"one"
    assert (output_dir / "labels/train/one.txt").read_text() == "1 0.500000 0.500000 0.500000 0.600000\n"
    ...
    with pytest.raises(FileExistsError, match="Pass --overwrite"):
        export_roboflow_yolo(frames_dir, frame_metadata_path, prediction_path, output_dir)
