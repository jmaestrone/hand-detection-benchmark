...
def export_head_left_videos(
    ...
    records_by_source = {
        str(record["source_mcap_path"]): record for record in existing_records
    }
    ...
    for mcap_path in mcap_paths:
        ...
        write_jsonl(
            metadata_path,
            sorted(
                records_by_source.values(),
                key=lambda record: str(record["source_mcap_path"]),
            ),
        )
        exported_count += 1
...
def extract_frames(
    ...
    provenance_by_video = {
        Path(record["output_video_path"]).name: record
        for record in read_jsonl(video_metadata_path)
    }
    ...
    for video_path in video_paths:
        ...
        frame_paths = [
            ...
            / build_frame_file_name(
                video_path.stem, frame_index, timestamp_seconds, image_format
            )
            for frame_index, timestamp_seconds in sample_points
        ]
        extract_video_frames(
            video_path, sample_points, frame_paths, image_format, quality, overwrite
        )
        for (frame_index, timestamp_seconds), frame_path in zip(
            sample_points, frame_paths, strict=True
        ):
            filename = build_frame_file_name(
                video_path.stem, frame_index, timestamp_seconds, image_format
            )
        ...
        write_jsonl(
            metadata_path,
            sorted(
                records_by_filename.values(),
                key=lambda record: str(record["file_name"]),
            ),
        )
...
def video_provenance(result: VideoExportResult, output_dir: Path) -> dict[str, object]:
    ...
    return asdict(
        VideoProvenance(
            ...
            output_video_path=result.output_video_path.relative_to(
                output_dir
            ).as_posix(),
...
def iter_sample_points(
    video_info: VideoInfo, fps: float
    ...
    for sample_number in range(sample_count):
        ...
        if timestamp >= video_info.duration_seconds or (
            video_info.frame_count is not None and frame_index >= video_info.frame_count
        ):
            break
...
def build_frame_file_name(
    video_stem: str, frame_index: int, timestamp_seconds: float, image_format: str
) -> str:
...
def extract_video_frames(
    ...
    selected_frames = "+".join(
        f"eq(n\\,{frame_index})" for frame_index, _ in sample_points
    )
    with tempfile.TemporaryDirectory(
        prefix="frame-extract-", dir=output_paths[0].parent
